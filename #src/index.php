<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/style.min.css">
    <title>Sub Sandwiches - Breakfast, Sandwiches, Salads & More | SUBWAY® | SUBWAY.com - United States (English)</title>
</head>
<body>
    <section class="inbetweener">
        <div class="inbetweener__container">
            @@include("_header.html")
            <div class="inbetweener__body">
                <div class="inbetweener__img"></div>
                <div class="inbetweener__contant">
                    <div class="promote">
                        <h1 class="promote__title">New Automation Tool for Your Home</h1>
                        <p class="promote__text">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Faucibus tristique vulputate ultrices ut mauris tellus at. Posuere sollicitudin odio tellus elit.</p>
                        <button class="promote__button"><span>See Our Project</span></button>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="showcase">
        <div class="showcase__container">
            <div class="showcase__columns">
                <h1 class="showcase__title">What we do to help our client grow in digital era,</h1>
                <div class="small-block">
                    <div class="small-block__flex">
                        <div class="small-block__img"></div>
                    </div>
                    <div class="small-block__info">
                        <h1 class="small-block__title">Make Your business To Be Better Famous In Internet</h1>
                        <p class="small-block__text">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. </p>
                        <div class="small-block__group">
                            <button class="small-block__button"><p>Learn more</p><span></span></button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="showcase__columns">
                <div class="small-block">
                    <div class="small-block__flex">
                        <div class="small-block__img"></div>
                    </div>
                    <div class="small-block__info">
                        <h1 class="small-block__title">Bring Technology To Your Comfrotable Home</h1>
                        <p class="small-block__text">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. </p>
                        <div class="small-block__group">
                            <button class="small-block__button"><p>Learn more</p><span></span></button>
                        </div>
                    </div>
                </div>
                <div class="small-block">
                    <div class="small-block__flex">
                        <div class="small-block__img"></div>
                    </div>
                    <div class="small-block__info">
                        <h1 class="small-block__title">Build Your Digital Product That Suitable For Your Need</h1>
                        <p class="small-block__text">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. </p>
                        <div class="small-block__group">
                            <button class="small-block__button"><p>Learn more</p><span></span></button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="basement">
        <div class="basement__rounding">
            <div class="basement__container">
                <div class="basement__heading">
                    <h1 class="basement__title">Our Beloved Client</h1>
                    <p class="basement__text">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam.</p>
                </div>
                <div class="basement__list">
                    <div class="basement__list-items"><p>logo client 1</p></div>
                    <div class="basement__list-items"><p>logo client 2</p></div>
                    <div class="basement__list-items"><p>logo client 3</p></div>
                    <div class="basement__list-items"><p>logo client 4</p></div>
                    <div class="basement__list-items"><p>logo client 5</p></div>
                    <div class="basement__list-items"><p>logo client 6</p></div>
                    <div class="basement__list-items"><p>logo client 7</p></div>
                    <div class="basement__list-items"><p>More client</p></div>
                </div>
            </div>
        </div>
    </section>
    <script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>
    <script src="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js"></script>
    <script src="scripts/script.min.js" defer></script>
</body>
</html>